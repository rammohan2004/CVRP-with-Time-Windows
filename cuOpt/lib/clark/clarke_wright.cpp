...
clarke_wright_cvrptw_parallel_v3(const VRP &vrp,
  ...
  // Split thread budget between outer (cluster) and inner (merge) levels
  int total_threads = omp_get_max_threads();
  int outer_threads = min(num_clusters, total_threads);
  int inner_threads = max(1, total_threads / outer_threads);
...
#pragma omp parallel for schedule(dynamic) num_threads(outer_threads)
  for (int c = 0; c < num_clusters; ++c) {
    ...
    auto verify_route_local = [&](const vector<node_t> &route) {
      return compute_arrival_time(route) >= 0;
    };
